...
const NavBar: React.FC = () => { // Define el componente funcional NavBar.
  ...
  const heightClass = 
    subItems.length === 1 ? 'h-[65px]' :
    subItems.length === 2 ? 'h-[130px]' :
    subItems.length === 3 ? 'h-[195px]' :
    subItems.length === 4 ? 'h-[260px]' :
    subItems.length === 5 ? 'h-[325px]' :
    subItems.length === 6 ? 'h-[390px]' :
    subItems.length === 7 ? 'h-[455px]' :
    subItems.length === 8 ? 'h-[520px]' :
    subItems.length === 9 ? 'h-[585px]' : 'h-65';
    ...
    <nav className="bg-[#210a3e] h-[70px] flex items-center justify-between relative hidden lg:flex px-[170px] z-20">
      ...
      <div className="flex items-center">
        ...
        {menuItems.map(item => (
          <div
            ...
            onMouseEnter={() => { handleMouseEnter(item.name); }} // Maneja el evento de hover.
          ...
          >
            <Button
              ...
              onClick={() => { handleItemClick(item.name); }} // Maneja el clic en el botón.
            >
              <Link href={item.href}>{item.name.toUpperCase()}</Link> {/* Enlaza al ítem. */}
            </Button>
                    ...
                    <Link key={subItem.name} href={subItem.href} passHref>
                      <div className="flex justify-start px-4 py-6 cursor-pointer z-10 whitespace-nowrap">
                        <span className="text-[#210a3e] hover:text-[#0d0d0d] transition-colors duration-300 hover:font-bold text-xs ml-6">
                          {subItem.name.toUpperCase()} {/* Muestra el nombre del subítem en mayúsculas */}
                        </span>
                      </div>
                    </Link>
                  ))}
      ...
      {(hoveredItem != null) && ((currentItem?.subItems) != null) && (
        <div className="absolute top-[70px] inset-x-0 z-10 bg-white">
          <div className={`${heightClass}`} /> {/* Clase de altura dinámica basada en la cantidad de subítems */}
